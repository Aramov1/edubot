...
class VelTrajectoryController(Node):
    def __init__(self, start_pos, offset=0.2, gain_pos=0.5, gain_rot = 0.2, max_joint_vel=0.6, max_cart_vel = 0.1):
        super().__init__('vel_trajectory_controller')
        
        # Trajectory Parameters
        self.start_pos = start_pos
        self.start_rot = [-np.pi/2, 0]  # Fixed orientation for the end-effector (facing down)
        ...
        self.gains = [gain_pos] * 3 + 2 * [gain_rot]  # Separate gains for position and orientation
    ...
    def check_within_workspace(self):
        ...
        while  not (conf_up and conf_down) and self.offset > 0:
            ...
            conf_up, conf_down = get_offset_ik(self.offset)
        self.offset = round(self.offset - 0.05, 3)
    ...
    def control_loop(self):
        ...
        if not self.elbow_up_reached:
            ...
            self.get_logger().info(f"Moving to start configuration... Joint error: {np.round(joint_error, 3)}")
            joint_error_dis = np.linalg.norm(joint_error)

            if joint_error_dis < 0.1:
            ...
            joint_vel = np.clip(self.gain_pos * joint_error, -self.max_joint_vel, self.max_joint_vel)
...
def main(args=None):
    ...
    node = VelTrajectoryController(start_pos=[0.0, 0.3, 0.2], offset=0.15)
    try:
        rclpy.spin(node)
