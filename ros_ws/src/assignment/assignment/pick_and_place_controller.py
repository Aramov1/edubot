...
# ---------------------------------------------------------------------------
# Hardware-validated joint configurations (from point_follower.py)
# Arm joints only [base, shoulder, upper_arm, lower_arm, wrist] — 5 DOF
# ---------------------------------------------------------------------------

Q_PICK   = np.array([-0.312,  0.428, -0.95,  -1.09,  -0.312])  # PICK 1 actual
Q_PLACE1 = np.array([-0.94,   0.357, -0.9,   -1.079, -0.914])  # PLACE 1 actual
Q_PLACE2 = np.array([-0.94,   0.428, -0.8,   -1.247, -0.94])   # PLACE 2 actual
Q_PLACE3 = np.array([-0.94,   0.466, -0.622, -1.406, -0.94])   # PLACE 3 actual
...
class LTPBTrajectory:
    ...
    Creates a smooth arc from P1 to P4: lift up, travel sideways, lower down.
    Includes gripper dwell time at each endpoint.
    """
    def __init__(self, P1, P4, lift_height, v_max=0.1, a_max=0.2, grasp=True):
        ...
        # Gripper dwell time at each endpoint so the gripper fully opens/closes
        self.t_grip = 1.0
        ...
        for t in self.time_array:
            if t < self.t_grip:
                ...
                ltpb_t = 0.0
                grip_val = GRIPPER_CLOSED if grasp else GRIPPER_OPEN
            elif t < self.t_grip + self.t7:
                # Phase 2: travel, hold gripper state
                ltpb_t = t - self.t_grip
                grip_val = GRIPPER_CLOSED if grasp else GRIPPER_OPEN
            else:
                # Phase 3: dwell at P4, release gripper
                ltpb_t = self.t7
                grip_val = GRIPPER_OPEN

            self.x_path.append(self.P1[0] + self._local_x(ltpb_t) * self.cos_theta)
            self.y_path.append(self.P1[1] + self._local_x(ltpb_t) * self.sin_theta)
            self.z_path.append(self.P1[2] + self._local_y(ltpb_t))
    ...
    def _local_x(self, t):
        """Horizontal distance travelled along the P1→P4 direction."""
        if t < self.t2:
            return 0.0
        elif t < self.t3:
            dt = t - self.t2
    ...
    def _local_y(self, t):
        """Vertical (Z) displacement: lift up then lower down."""
        if t < self.t1:
...
class PickAndPlaceActionServer(Node):
    ...
    def __init__(self):
        ...
        self.Kp = 2.0        # P-control gain for init phase
        self.max_vel = 0.5
        self.tolerance = 0.05
...
class PickAndPlaceActionClient(Node):
    """
    ROS 2 Action Client.
...
def main_client(args=None):
    rclpy.init(args=args)
    node = PickAndPlaceActionClient()

    # 3 pick-and-place cycles: always pick from the same spot, place at 3 locations
    sequence = [
    ...
    node.get_logger().info('Pick-and-place sequence complete. Shutting down client.')
    node.destroy_node()
    rclpy.shutdown()


# Default entry point runs the server (matches vel_trajectory_controller pattern)
def main(args=None):
    main_server(args)
